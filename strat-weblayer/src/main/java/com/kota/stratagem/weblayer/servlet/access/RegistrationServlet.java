...
public class RegistrationServlet extends HttpServlet implements RegistrationParameter, RegistrationAttribute {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		final AppUserRepresentor user = new AppUserRepresentor("", "", "", RoleRepresentor.PRISTINE_USER, null, null, 0, 0);
	...
	protected void doPost(final HttpServletRequest request, final HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			if (((username == null) || "".equals(username)) || ((password == null) || "".equals(password)) || !(password.equals(password_confirmation))) {
				...
				final AppUserRepresentor user = new AppUserRepresentor(username, "", email, RoleRepresentor.PRISTINE_USER, null, null, 0, 0);
				this.forward(request, response, user, false);
			} else {
				AppUserRepresentor user = null;
				try {
					LOGGER.info("Registration successful for user: " + username);
					user = this.protocol.saveAppUser(null, username, password, email, RoleRepresentor.PRISTINE_USER, null);
