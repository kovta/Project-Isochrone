...
public class TaskActionController extends AbstractRefinerServlet implements TaskAttribute, TaskParameter {
	...
	private void forward(final HttpServletRequest request, final HttpServletResponse response, final TaskRepresentor task, final boolean editFlag, String returnPoint, boolean errorFlag)
		...
		try {
			...
			if((name == null) || "".equals(name)) {
				...
				try {
					...
					task = this.taskProtocol.saveTask(id, name, description, priority, completion, deadline, admittance, request.getUserPrincipal().getName(), objective_id, project_id, submodule_id,
	...
	private boolean isCompleteEstimationProvided(final HttpServletRequest request) {
