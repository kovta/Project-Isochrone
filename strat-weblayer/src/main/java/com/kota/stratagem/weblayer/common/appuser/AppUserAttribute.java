package com.kota.stratagem.weblayer.common.appuser;

public interface AppUserAttribute {

	public static final String ATTR_APPUSER = "user";
	public static final String ATTR_SUBORDINATE_USER = "subordinateUser";
	public static final String ATTR_SUBORDINATE_ROLES = "subordinateRoles";
	public static final String ATTR_USER_ERROR = "usr-error";
	public static final String ATTR_USER_SUCCESS = "usr-success";

}
