package com.kota.stratagem.weblayer.common.objective;

import com.kota.stratagem.weblayer.common.assignment.AssignableAttribute;

public interface ObjectiveAttribute extends AssignableAttribute {

	public static final String ATTR_OBJECTIVE = "objective";
	public static final String ATTR_NEW_OBJECTIVE = "newObjective";
	public static final String ATTR_ERROR = "obj-error";
	public static final String ATTR_SUCCESS = "obj-success";

}
